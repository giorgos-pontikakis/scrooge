  ...
  (labels ((invoice-receivable/payable-account (kind)
           ...
           (invoice-base-where (direction kind)
             `((:= ,(invoice-receivable/payable-account kind)
                   ,(invoice-receivable/payable-root direction))
               (:in ,(invoice-revenues/expenses-account kind)
                    (:set ,@(invoice-revenues/expenses-set direction))))))
...
(defun invoice-incoming-p (direction)
  (string-equal direction "incoming"))

(defun invoice-debit-p (kind)
  (string-equal kind "debit"))

(defun invoice-revenues/expenses-set (direction)
  (if (invoice-incoming-p direction) *revenues-accounts* *expense-accounts*))

(defun invoice-revenues/expenses-root (direction)
  (if (invoice-incoming-p direction)
      (account-id 'revenues-root-account)
      (account-id 'expenses-root-account)))

(defun invoice-receivable/payable-root (direction)
  (if (invoice-incoming-p direction)
...
(defun invoice-page-title (direction kind op-label)
  (let ((kind-label
        ...
        (direction-label
          (if (invoice-incoming-p direction) "Πελάτες" "Προμηθευτές")))
